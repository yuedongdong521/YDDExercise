...
@implementation YDDTextScrollView
...
{
    if (content.length > 0) {
        CGFloat width =  [self widthWithHeight:self.height andFont:_fontSize object:content].width;
...
- (void)initWithSubViews {
    ...
    for (int i = 0; i < self.dataArray.count; i++) {
        CGFloat width =  [self widthWithHeight:self.height andFont:_fontSize object:self.dataArray[i]].width;
        _width = width + _width;
    }
    ...
    for (int i = 0; i < self.dataArray.count; i++) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(labelX, 0, width, self.height)];
    ...
    _scrollViewWidth = _width + (self.dataArray.count - 1) * self.inteval;
    self.contentSize = CGSizeMake(_scrollViewWidth, self.height);
}
...
- (void)timerStart {
    ...
    CGFloat lastWidth = [self widthWithHeight:self.height andFont:_fontSize object:self.dataArray[0]].width;
    CGFloat firstWidth = [self widthWithHeight:self.height andFont:_fontSize object:[self.dataArray lastObject]].width;
