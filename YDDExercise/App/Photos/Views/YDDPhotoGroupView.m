...
@interface YDDPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate, PhotoGroupCellDelegate>
...
@implementation YDDPhotoGroupView
...
{
    if ([_delegate respondsToSelector:@selector(YDDPhotoGroupView:rightActionWithPage:)]) {
        [_delegate YDDPhotoGroupView:self rightActionWithPage:self.currentIndex];
    }
}
...
{
    if ([_delegate respondsToSelector:@selector(YDDPhotoGroupView:getThumbViewWithPage:)]) {
        return [_delegate YDDPhotoGroupView:self getThumbViewWithPage:page];
    }
    return nil;
}
...
{
    if ([_delegate respondsToSelector:@selector(YDDPhotoGroupView:getImageWithPage:)]) {
        return [_delegate YDDPhotoGroupView:self getImageWithPage:page];
    }
    return nil;
}
...
//            CGFloat moveScale = scale > 0.6 ? scale : 0.6;
            ...
            NSLog(@"deltaY : %f, movePY = %f", deltaY, moveP.y);
