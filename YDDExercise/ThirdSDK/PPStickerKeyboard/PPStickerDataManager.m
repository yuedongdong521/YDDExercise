...
//    NSString *path = [NSBundle.mainBundle pathForResource:@"InnerStickersInfo" ofType:@"plist"];
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"emoticon" ofType:@"plist"];
    if (!path) {
        return;
    }

    NSArray *array = [[NSArray alloc] initWithContentsOfFile:path]; //所有的数据数组
...
//        NSArray *emojiArr = stickerDict[@"emoticons"];
        ...
        for (NSDictionary *emojiDict in array) {
            PPEmoji *emoji = [[PPEmoji alloc] init];
            
            emoji.imageName = emojiDict[@"png"];
            emoji.emojiDescription = emojiDict[@"cht"];
...
- (NSArray<PPStickerMatchingResult *> *)matchingEmojiForString:(NSString *)string
{
    ...
    if (results && results.count) {
        NSMutableArray *emojiMatchingResults = [[NSMutableArray alloc] init]; //表情匹配结果
...
//                emojiMatchingResult.emojiImage = [UIImage imageNamed:[@"Sticker.bundle" stringByAppendingPathComponent:emoji.imageName]];
                ...
                emojiMatchingResult.emojiImage = image;
