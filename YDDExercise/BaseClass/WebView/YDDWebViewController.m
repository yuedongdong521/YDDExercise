...
- (void)viewDidLoad {
    ...
    [self.webView startLoadWithUrl:self.urlStr];
}

- (NSString *)urlScheme
{
    return [NSURL URLWithString:self.urlStr].scheme;
}
...
- (void)webView:(YDDWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if ([urlStr isEqualToString:webView.currentURL] || [urlStr hasPrefix:self.urlStr]) {
        self.needGoBack = NO;
    } else {
        self.needGoBack = YES;
    }
    ...
    if ([scheme isEqualToString:@"http"] || [scheme isEqualToString:@"https"] || [scheme isEqualToString:@"file"]) {
        ...
        if ([urlStr hasPrefix:@"https://wx.tenpay.com/cgi-bin/mmpayweb-bin/checkmweb"] &&
            ![urlStr hasSuffix:[NSString stringWithFormat:@"redirect_url=%@",self.urlScheme]] &&
            ...
            if ([urlStr containsString:@"redirect_url="]) {
                ...
                redirectUrl = [[urlStr substringToIndex:redirectRange.location] stringByAppendingString:[NSString stringWithFormat:@"redirect_url=%@",self.urlScheme]];
            }else {
                redirectUrl = [urlStr stringByAppendingString:[NSString stringWithFormat:@"&redirect_url=%@",self.urlScheme]];
            }
