...
@interface YDDWaterfallFlowLayout ()

//该委托设置为self.collectionView.delegate
@property (nonatomic,weak) id<YDDWaterfallFlowLayoutDelegate> delegate;
//存所有布局
...
@implementation YDDWaterfallFlowLayout
...
- (id<YDDWaterfallFlowLayoutDelegate>)delegate
{
    
    if (!_delegate) {
        _delegate = (id<YDDWaterfallFlowLayoutDelegate>)self.collectionView.delegate;
    }
    return _delegate;
}
